...
class model_runner():
    ...
    def process_img(self, img0,  offset_x = 0, offset_y = 0):
        # so now gen is compatible with next -> next(gen)
        gen = patch_generator(img0, patch_size=self.patch_size) # gonna make a new patch generator every time.

        # tuples are hashable so you can make a default dict out of this.  
        out = defaultdict(float)
        
        with torch.no_grad():
            for patch, y, x in gen:
                patch_ = torch.from_numpy(patch).permute(2,0,1).float() # permuting a lot here... maybe permute whole image first?
                patch_ = self.transforms(patch_)
                patch_ = patch_.unsqueeze(0).to(self.device)

                model_pred = self.model(patch_) # [1, num_classes]
                # print('y_outputshape ', y.shape)
                probs = torch.softmax(model_pred, dim=1) # probs is still a gpu object
                score = probs[0, 1].item() # pulls to cpu scalar
    
                # hard threshold
                score = score if score >self.thr else 0.0
                
                new_y = int(y + self.c + offset_y) # consider doing top left corner instead of center. 
                new_x = int(x + self.c + offset_x)
               
                if score >0: # and if this doesnt run...should be fine. 
                    # key is the new point tuple value is the score. 
...
def mapmaker(padimg0, accumulated, patch_size=128):
    H, W = padimg0.shape[:2]
    ...
    for (y, x), v in accumulated.items():
        y0 = max(0, y - c)
        y1 = min(H, y + c)
        x0 = max(0, x - c)
        x1 = min(W, x + c)
